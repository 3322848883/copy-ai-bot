...
| 前台 | 登录 / 注册 | 邮箱注册 + 6 位验证码（5 分钟）、好友邀请码选填（限注册后 24h 内绑定）、交易所邀请码选填（注册后可补填，管理员复核通过后免订阅）、防自邀防循环 |
...
| 前台 | 个人中心 | API Key 绑定/解绑、好友邀请码绑定（24h 时限）、交易所邀请码补填（待复核/已通过/已驳回状态展示）、交易所选择 |
| 前台 | 订阅 | 试用（5U 仅 1 次）/ 正式（19.9U）套餐、多链 USDT 支付、自动核实 |
| 后台 | 12 模块 | 用户/信号源/跟单订单/主号下级审核/钱包账本/邀请奖励/支付订单/提现审核/风控/审计日志/交易所邀请码（预设 + 用户绑定复核）/信号会话 |
...
- **双轨信号源**：模式 A 爬虫（Gate 带单广场，3-8s）/ 模式 B 跟单账户 WS 镜像（1.5-3s），信号清洗标准化，延迟红线 A>10s、B>5s 丢弃（**close/reduce 风险释放动作豁免红线**，避免无界敞口）。
- **跟单引擎**：USDT 本位换算 4 步法、独立虚拟账本隔离、8 类失败归因、币种/精度/余额风控；跟单阀门 `follow_enabled` 由后台管理员显式控制，隐藏仓位策略默认不可跟单。
- **订阅变现**：试用/正式双套餐、多链支付自动核实（即时 3 项 + 4 次轮询）、到期禁开仓加仓、试用限购数据库强校验；**合作归属免订阅**（平台池主号下级 / 交易所邀请码管理员复核通过）。
- **邀请奖励**：10% 奖励 + 24h 延迟核实（防退款回滚）、1h 批量 5U 防刷检测、负数冻结提现；好友码限注册后 24h 内绑定（不允许事后补填）。
...
| M8 | 跟单阀门后台化（follow_enabled）+ 邀请码选填与管理员复核免订阅 + 信号采集稳定性与延迟优化 | ✅ 已完成 |
...
- M8 功能变更（跟单阀门 + 邀请码复核免订阅 + 采集稳定性与延迟优化）：`2026-08-24-m8-follow-valve-invite-review.md`
